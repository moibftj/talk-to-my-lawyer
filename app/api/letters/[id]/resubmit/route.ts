  ...
  try {
    ...
    if (letter.status !== "rejected") {
      return errorResponses.validation(
        "Only rejected letters can be resubmitted",
      );
    }
    ...
    if (!deductionResult.success) {
      return errorResponses.forbidden(
        deductionResult.errorMessage ||
          "No letter credits remaining. Please upgrade your plan.",
      );
    }

    const shouldRefund =
      !deductionResult.isFreeTrial && !deductionResult.isSuperAdmin;

    // Capture rejection reason BEFORE clearing it
    const rejectionFeedback =
      letter.rejection_reason || "No specific feedback provided";
    ...
    try {
      // Regenerate letter with rejection feedback (captured before clearing)
      const prompt = buildResubmitPrompt(letter, rejectionFeedback);

      const { text: generatedContent } = await generateText({
        model: getOpenAIModel("gpt-4-turbo"),
        system:
          "You are a professional legal attorney revising a formal legal letter based on feedback. Incorporate the rejection feedback to create an improved, professional letter.",
        prompt,
        temperature: 0.7,
