...
import { handleApiError, errorResponses } from "@/lib/api/api-error-handler";
...
export async function POST(
  ...
  try {
    ...
    // Fetch letter and validate status transition
    ...
    // Status guard: validate the action is valid for the current letter status
    const validSourceStatuses: Record<string, string[]> = {
      edit_content: ["draft", "pending_review", "under_review"], // Can edit pre-approval
      approve: ["pending_review", "under_review"],
      reject: ["pending_review", "under_review"],
      mark_completed: ["approved"],
    };
    const allowedStatuses = validSourceStatuses[action] || [];
    if (!allowedStatuses.includes(currentLetter.status)) {
      return NextResponse.json(
        {
          error: `Cannot ${action} a letter with status '${currentLetter.status}'. Requires: ${allowedStatuses.join(" or ")}`,
        },
        { status: 409 },
      );
    }
    ...
    switch (action) {
      ...
      case "approve": {
        ...
        auditNotes =
          sanitizeResult.sanitized.reviewNotes || "Letter approved by admin";
        shouldNotify = true;
    ...
    return handleApiError(error, "Admin Letter Update");
  }
}
