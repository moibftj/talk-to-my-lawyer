...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (couponCode) {
      ...
      console.log(
        "[Checkout] Validating coupon with fraud detection:",
        couponCode,
      );
      const couponValidation = await validateCouponWithFraudDetection(
        couponCode,
        request,
        user.id,
      );
    ...
    if (TEST_MODE) {
      console.log(
        "[Checkout] TEST MODE: Simulating payment for user:",
        user.id,
      );
      ...
      if (atomicError || !atomicResult || !atomicResult[0]?.success) {
        console.error(
          "[Checkout] TEST MODE: Atomic subscription creation failed:",
          atomicError,
        );
        const errorMsg =
          atomicResult?.[0]?.error_message ||
          atomicError?.message ||
      ...
      return NextResponse.json({
        ...
        message:
          "TEST MODE: Subscription created successfully (simulated payment)",
    ...
    if (subError) {
      console.error(
        "[Checkout] Failed to create pending subscription:",
        subError,
      );
