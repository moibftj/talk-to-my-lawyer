...
export default async function LetterDetailPage({
  ...
  const timelineSteps = [
    ...
    },
    {
      ...
      status: ["approved", "rejected"].includes(letter.status)
        ? "completed"
        : "pending",
      icon:
  ...
  const showReviewModal =
    submittedParam ||
    ["pending_review", "under_review", "generating"].includes(letter.status);
      ...
      <div className="max-w-4xl mx-auto">
        <div className="mb-6">
          <Link
            ...
            className="text-primary hover:text-primary/80 text-sm flex items-center gap-1 mb-4"
          >
            <svg
              className="w-4 h-4"
              fill="none"
          ...
          <div className="flex justify-between items-start">
            <div>
              ...
              <div className="flex items-center gap-4 text-sm text-muted-foreground">
                <span>
                  Created {format(new Date(letter.created_at), "MMM d, yyyy")}
                </span>
                <span
                  className={`px-2 py-1 rounded-full text-xs font-semibold ${statusColors[letter.status]}`}
                >
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-warning/80">
                    Your letter is being reviewed by our legal team. You'll be
            ...
            <div className="bg-white rounded-lg shadow-sm border p-6 space-y-4">
              ...
              {profile.role === "admin" && letter.status !== "approved" && (
                <div className="bg-amber-50 border border-amber-200 rounded p-3 text-sm text-amber-800">
                  <strong>Admin Note:</strong> This draft is hidden from the
                  user until approval.
                </div>
              )}
            ...
            letter.status !== "rejected" && (
              <div className="bg-white rounded-lg shadow-sm border p-6 space-y-4">
                ...
                <div className="bg-muted/30 border rounded-lg p-8 text-center text-muted-foreground">
                  ...
                  <p className="text-sm">
                    Your letter draft is currently under attorney review.
                  </p>
                  <p className="text-sm mt-1">
                    The content will be visible here once approved.
                  </p>
                </div>
              </div>
            )}
