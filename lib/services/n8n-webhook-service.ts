...
export const n8nConfig: N8nConfig = {
  ...
  // Retries are disabled to ensure a single generateLetterViaN8n call cannot exceed the route's maxDuration.
  timeout: 55000,
  maxRetries: 0,
};
...
export async function generateLetterViaN8n(
  ...
  if (!n8nConfig.isConfigured || !n8nConfig.webhookUrl) {
    throw new Error(
      "n8n webhook is not configured. Set N8N_WEBHOOK_URL environment variable.",
    );
  }

  const webhookUrl = n8nConfig.webhookUrl;
  console.log(
    "[n8n] Sending letter generation request for:",
    formData.letterId,
  );
  ...
  for (let attempt = 1; attempt <= n8nConfig.maxRetries; attempt++) {
    try {
      ...
      console.log(
        "[n8n] Sending payload with letterType:",
        formData.letterType,
        "intakeData keys:",
        Object.keys(formData.intakeData),
      );
      ...
      if (!response.ok) {
        if (response.status === 404) {
          throw new Error(
            "n8n workflow not found. Ensure the workflow is active and listening.",
          );
        }
        ...
        if (response.status >= 500) {
          console.warn(
            `[n8n] Server error (${response.status}), attempt ${attempt}/${n8nConfig.maxRetries}`,
          );
      ...
      if (error instanceof Error && error.name === "AbortError") {
        console.warn(
          `[n8n] Request timeout, attempt ${attempt}/${n8nConfig.maxRetries}`,
        );
        if (attempt < n8nConfig.maxRetries) {
          const delay = Math.pow(2, attempt) * 1000;
          await new Promise((resolve) => setTimeout(resolve, delay));
...
export async function generatePdfViaN8n(
  ...
  if (!n8nPdfConfig.isConfigured || !n8nPdfConfig.webhookUrl) {
    throw new Error(
      "n8n PDF webhook is not configured. Set N8N_PDF_WEBHOOK_URL environment variable.",
    );
  }
  ...
  for (let attempt = 1; attempt <= n8nPdfConfig.maxRetries; attempt++) {
    try {
      ...
      if (!response.ok) {
        if (response.status === 404) {
          throw new Error(
            "n8n PDF workflow not found. Ensure the workflow is active and listening.",
          );
        }
        ...
        if (response.status >= 500) {
          console.warn(
            `[n8n-pdf] Server error (${response.status}), attempt ${attempt}/${n8nPdfConfig.maxRetries}`,
          );
        ...
        const errorText = await response.text();
        throw new Error(
          `n8n PDF request failed (${response.status}): ${errorText}`,
        );
      }
      ...
      if (!result.success) {
        throw new Error(
          result.error || result.message || "n8n PDF generation failed",
        );
      }

      console.log(
        "[n8n-pdf] PDF generated successfully for:",
        params.letterId,
        {
      ...
      if (error instanceof Error && error.name === "AbortError") {
        console.warn(
          `[n8n-pdf] Request timeout, attempt ${attempt}/${n8nPdfConfig.maxRetries}`,
        );
        if (attempt < n8nPdfConfig.maxRetries) {
          const delay = Math.pow(2, attempt) * 1000;
          await new Promise((resolve) => setTimeout(resolve, delay));
