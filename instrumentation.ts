...
async function initStripe() {
  try {
    ...
    if (!databaseUrl) {
      console.warn(
        "[Instrumentation] DATABASE_URL not set, skipping Stripe sync initialization",
      );
      return;
    }

    const isReplit = Boolean(
      process.env.REPL_ID || process.env.REPLIT_CONNECTORS_HOSTNAME,
    );

    if (isReplit) {
      // Use dynamic module name to prevent Turbopack from analyzing this at build time
      ...
      try {
        ...
        console.warn(
          "[Instrumentation] stripe-replit-sync not available, skipping:",
          e,
        );
        return;
      }
      ...
      if (domain) {
        ...
        console.log(
          "[Instrumentation] Stripe managed webhook configured:",
          webhookUrl,
        );
      } else {
        console.warn(
          "[Instrumentation] REPLIT_DOMAINS not set, skipping managed webhook setup",
        );
      }
      ...
      stripeSync
        ...
        .catch((error: unknown) => {
          console.error(
            "[Instrumentation] Stripe backfill sync failed:",
            error,
          );
        });
    } else {
      console.log(
        "[Instrumentation] Non-Replit environment detected, using standard Stripe webhook (configure STRIPE_WEBHOOK_SECRET)",
      );
    }
  } catch (error) {
